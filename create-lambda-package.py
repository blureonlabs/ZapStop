...
def create_lambda_package():
    """Create a Lambda deployment package with psycopg2"""
...
'''
    ...
    with open("lambda-package/index.py", "w") as f:
        f.write(lambda_code)
    
    print("✅ Lambda function code created")
    
    # Install psycopg2-binary in the package directory
    print("📦 Installing psycopg2-binary...")
    subprocess.run([
        sys.executable, "-m", "pip", "install", 
        "psycopg2-binary", "-t", "lambda-package"
    ], check=True)
    ...
    with zipfile.ZipFile("lambda-deployment.zip", "w", zipfile.ZIP_DEFLATED) as zipf:
        for root, dirs, files in os.walk("lambda-package"):
            for file in files:
                ...
                arc_path = os.path.relpath(file_path, "lambda-package")
                zipf.write(file_path, arc_path)
    
    print("✅ Deployment package created: lambda-deployment.zip")
