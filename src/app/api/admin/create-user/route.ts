...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!['admin', 'accountant', 'driver'].includes(role)) {
      return NextResponse.json({ error: 'Invalid role. Must be admin, accountant, or driver' }, { status: 400 })
    }

    if (!supabaseAdmin) {
      return NextResponse.json({ 
        error: 'Admin client not configured. Please add SUPABASE_SERVICE_ROLE_KEY to your environment variables.' 
      }, { status: 500 })
    }
