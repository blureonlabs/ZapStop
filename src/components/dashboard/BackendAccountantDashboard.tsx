...
export default function BackendAccountantDashboard() {
  ...
  const loading = expensesLoading || earningsLoading || usersLoading
  ...
  const expenseStats = useMemo(() => {
    if (!expenses) return { total: 0, pending: 0, approved: 0, rejected: 0, totalAmount: 0, approvedAmount: 0 }
  ...
  const driverStats = useMemo(() => {
    if (!drivers || !earnings || !expenses) return []
